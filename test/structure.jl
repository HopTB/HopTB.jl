...
let
    ...
    tm = get_Kane_Mele()
    Hop.Structure.permute_orbits!(tm, [2, 4, 1, 3])
    ...
    @test !Hop.has_full_information(tm)
end


let
    tm = get_Kane_Mele()
    slab = Hop.Structure.makeslab(tm, 1, 2)
    @test slab.norbits == 8
    ...
    @test slab.site_positions ≈ Hop.get_orbital_position(slab)[:, 1:4]
...
let
    tm_ref = get_Kane_Mele()
    tm = get_Kane_Mele()
    Hop.Structure.shift!(tm, [0.1, 0, 0])
    @test tm.site_positions ≈ Hop.get_orbital_position(tm)[:, 1:2]
...
let
    tm = get_Kane_Mele()
    @test Hop.Structure.make_superlattice(tm, [1 0 0; -1 2 0; 0 0 1]')[1][:, 1] == [-1, 0, 0]
    sc = Hop.Structure.make_supercell(tm, [1 0 0; -1 2 0; 0 0 1]')
    ...
    @test Hop.get_orbital_position(sc)[:, 1] ≈ Hop.get_orbital_position(tm)[:, 2]-tm.lat[:, 1]
end


let
    tm = getBN()
    sctm = Hop.Structure.make_supercell(tm, copy([1 0 0; -1 2 0; 0 0 1]'))
    slab = Hop.Structure.makeslab(tm, 1, 2)
